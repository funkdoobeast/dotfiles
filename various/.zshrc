...
PS1="[%(!.${PR_RED}%n.$PR_LIGHT_CYAN%n)%(!.${PR_LIGHT_YELLOW}@.$PR_NO_COLOR@)$PR_NO_COLOR%(!.${PR_LIGHT_RED}%U%m%u.${PR_LIGHT_BLUE}%U%m%u)$PR_NO_COLOR:%(!.${PR_RED}%2c.${PR_LIGHT_BLUE}%2c)$PR_NO_COLOR]%(?..[${PR_LIGHT_RED}%?$PR_NO_COLOR])%(!.${PR_LIGHT_RED}#.${PR_LIGHT_GREEN}$) "
RPS1="$PR_LIGHT_CYAN(%D{%m-%d %H:%M:%S})$PR_NO_COLOR"
unsetopt ALL_EXPORT
